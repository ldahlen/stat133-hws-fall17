...
Combinations are typically expressed in terms facof factorials as:
...
same probability (2 heads in 5 tosses) of a __biased__ coin with a chance of heads
of 35%?
```{r}
dbinom(3, 5, .35, log = FALSE)
```

- Finally, obtain the probability of getting more than 3 heads in 5 tosses
with a biased coin of 35% chance of heads.
```{r}
dbinom(2, 5, .35, log = FALSE)
```
...
```{r}
#The tab _Files_ is good for creating new R Script/Markdown, saving, closing, and quiting.
#The tab _Help_ is good for getting more help about using R Script and R Studio.
#the tab _Files_ is good for organizing files saved on the computer.
#The tab _Help is useful for looking up functions in R.
#In the tab _Help_, cliking on the House icon shows a page with R/RStudio resources, manuels, and other helpful overviews
#The tab _History_ is useful for looking up previous lines and also moving certain lines of code to the console or the source.
#The tab _Environment_ gives an overview of all values created and allows you to search for objects.
```
...
2x<-2*x
```{r}
#I guess that there won't be an output
```
...
sqrt4
```{r}
#I guess that the output will be 2
```
...
a number <- 16
```{r}
#I guess that there will be no output
```
...
```
```{r}
#I guess that the output will be 16
```
...
```{r}
#The purpose of the _Source_ pane is to download things and use R Markdown.
#The purpose of the _Console_ pane is to type r commands
# The Purpose of the _Environment, History, etc_ pane is to help keep track of what you've already defined
#The purpose of the _Files, Plots, Packages, Help, Viewer_ pane is to visualize things and search for help in R.
```
